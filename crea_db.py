...
df= pd.read_csv(r"C:\Users\JuliaElenaSilloCondo\Downloads\proveITSAR.catasto_ssg.csv")
geodf = gpd.read_file(r"C:\Users\JuliaElenaSilloCondo\Downloads\comune_ssg.geojson", endcode= 'utf-8')

df_pulito = df.loc[:, ['nome', 'cognome', 'data_nascita', 'cod_fisc']]
df_pulito.set_index('nome')
df_pulito.index.name = "_id"
geodf.set_index('osm_id')
geodf.index.name = "_id"

merge_df=geodf.merge(df_pulito, how='left', on='_id')
merge_df.to_file('catasto_db.geojson',driver='GeoJSON')
print(merge_df)
